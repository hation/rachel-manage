...
class CinemaController extends BaseController
{
    ...
    public function cinemaIsValid()
    {
        ...
        $ex = M($this->CINEMAS_TABLE)->where(array('name' => $name))->find();
        if ($ex)
            $ex['flag'] = 1;
        else {
            $ex['flag'] = 0;
    ...
    public function addCinema()
    {
        ...
        $res = M($this->CINEMAS_TABLE)->add($data);
    ...
    {
        $data['id'] = I('post.id');
        $res = M($this->CINEMAS_TABLE)->where($data)->find();
        if ($res) {
            $res['isValidCinema'] = 1;
        } else {
            $res['isValidCinema'] = 0;
    ...
    public function updateCinema()
    {
        ...
        $res = M($this->CINEMAS_TABLE)->save($data);
    ...
    {
        $id = I('get.id');
        $r = M($this->CINEMAS_TABLE)->where(array('id' => $id))->delete();
        if ($r) {
            $this->success('删除成功！');
        } else {
            $this->error('删除失败！');
